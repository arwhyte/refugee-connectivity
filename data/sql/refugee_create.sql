...
CREATE TABLE IF NOT EXISTS `refugee`.`camp` (
  ...
  `camp_admin_name` VARCHAR(45) NULL,
...
INSERT IGNORE INTO camp (camp_name, camp_file_name, country_id, camp_admin_id, camp_admin_name, camp_type_id,longitude, latitude, bandwidth_up, bandwidth_down)
SELECT tc.camp_name, tc.camp_file_name,country.country_id,ca.camp_admin_id,tc.camp_admin,ct.camp_type_id,tc.longitude,tc.latitude,
       tc.bandwidth_up, tc.bandwidth_down
  FROM temp_camp tc
       LEFT JOIN country
              ON tc.country = country.country_name
       LEFT JOIN camp_admin ca
              ON tc.camp_admin = ca.camp_admin_name
...
INTO TABLE temp_appu
  ...
  -- LINES TERMINATED BY '\r\n'
  IGNORE 1 LINES
  (@dummy, @dummy, @dummy, application_category, total_usage_kB, cmy);
...
INTO TABLE temp_dupc
  ...
  -- LINES TERMINATED BY '\r\n'
  IGNORE 1 LINES
  (@dummy, @dummy, @dummy, @the_date, total_clients, cmy, dupc)
	SET date_ = str_to_date(@the_date,'%m/%d/%Y');
...
INSERT IGNORE INTO daily_usage_per_camp
  (mupc_id,date_,total_clients,dupc)
SELECT mupc.mupc_id, td.date_, td.total_clients,td.dupc
  FROM temp_dupc td
       LEFT JOIN monthly_usage_per_camp mupc
              ON td.cmy = mupc.cmy
 ORDER BY td.id;
...
-- -----------------------------------------------------
-- Table `refugee`.`hourly_usage_per_camp`
-- -----------------------------------------------------
CREATE TEMPORARY TABLE temp_hupc
  (
    id INT NOT NULL AUTO_INCREMENT UNIQUE,
    dupc VARCHAR(45) NOT NULL,
    time_ VARCHAR(45) NOT NULL,
    total_usage_kB INT NULL DEFAULT NULL,
    PRIMARY KEY (id)
  )
ENGINE=InnoDB
CHARACTER SET utf8mb4
...
LOAD DATA LOCAL INFILE '/Users/user/Desktop/si664/refugee-connectivity/data/csv/hourly_usage_per_camp.csv'
INTO TABLE temp_hupc
  CHARACTER SET utf8mb4
  -- FIELDS TERMINATED BY '\t'
  FIELDS TERMINATED BY ','
  -- ENCLOSED BY '"'
  ...
  -- LINES TERMINATED BY '\r\n'
  IGNORE 1 LINES
  (@dummy, @dummy, @dummy, @dummy, @the_time, total_usage_kB, @dummy, dupc)
	SET time_ = time(str_to_date(@the_time,'%H:%i'));

DROP TABLE IF EXISTS `refugee`.`hourly_usage_per_camp` ;

CREATE TABLE IF NOT EXISTS `refugee`.`hourly_usage_per_camp` (
  `hupc_id` INT(11) NOT NULL AUTO_INCREMENT UNIQUE,
  `dupc_id` INT(11) NOT NULL,
  `time_` TIME NOT NULL,
  `total_usage_kB` FLOAT NULL DEFAULT NULL,
  PRIMARY KEY (`hupc_id`),
  CONSTRAINT `dupc_id`
    FOREIGN KEY (`dupc_id`)
    REFERENCES `refugee`.`daily_usage_per_camp` (`dupc_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

INSERT IGNORE INTO hourly_usage_per_camp
  (dupc_id,time_,total_usage_kB)
SELECT dupc.dupc_id, th.time_, th.total_usage_kB
  FROM temp_hupc th
       LEFT JOIN daily_usage_per_camp dupc
              ON th.dupc = dupc.dupc
 ORDER BY th.id;

DELETE FROM hourly_usage_per_camp WHERE dupc_id=0;

DROP TEMPORARY TABLE temp_hupc;
